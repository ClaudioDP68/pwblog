...
public class AuthenticationResource {
    ...
    public Response login(@FormParam("email") String email, @FormParam("pwd") String pwd) {
        User found = store.findByUserAndPwd(email, pwd).orElseThrow(() -> new NotAuthorizedException("invalid user or password", Response.status(Response.Status.UNAUTHORIZED).build()));
